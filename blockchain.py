...
def get_balance(participant):
    ...
    amount_sent = functools.reduce(
        lambda tx_sum, tx_amt: tx_sum + sum(tx_amt) if len(tx_amt) > tx_sum + 0 else tx_sum, tx_sender, 0)

    tx_recipient = [[tx['amount'] for tx in block['transactions']
                     if tx['recipient'] == participant] for block in blockchain]
    amount_received = functools.reduce(
        lambda tx_sum, tx_amt: tx_sum + sum(tx_amt) if len(tx_amt) > tx_sum + 0 else tx_sum, tx_recipient, 0)
...
def verify_transaction(transaction):
    """ Verify a transaction by checking if the sender has enough coins.
...
def mine_block():
    ...
    reward_transaction = {'sender': 'MINING',
                          'recipient': owner, 'amount': MINED_REWARD}
...
def verify_chain():
    ...
    for (index, block) in enumerate(blockchain):
        ...
        if index == 0:
            # I dont want to validate that first block its the genesis block, we got no previous block here
            continue
